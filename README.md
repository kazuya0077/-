...
3. メニュー **公開 → ウェブアプリケーションとして導入** を選択し、
   ...
   として導入します。
4. 発行後に表示されるウェブアプリの URL を、プロジェクトルートの `config.js`（または `config.sample.js` をコピーしたファイル）の `window.APPS_SCRIPT_URL` に設定します。
